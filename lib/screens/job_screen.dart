...
class _JobScreenState extends State<JobScreen> {
  int _selectedIndex = 0; // 0 = คำขอจ้างงาน, 1 = กำลังดำเนินงาน, 2 = ประวัติการจ้าง, 3 = ยกเลิก/ปฏิเสธ
      ...
      builder: (context, fontProvider, child) {
        return Column(
          children: [
            ...
            Expanded(
              child: _buildTabContent(fontProvider),
            ),
          ],
        );
      },
  ...
  Widget _buildTabButtons(FontSizeProvider fontProvider) {
    ...
    return Container(
      ...
      height: 50, // ลดความสูงเหลือบรรทัดเดียว
          ...
          children: tabs.asMap().entries.map((entry) {
            ...
            return Padding(
              padding: EdgeInsets.only(
                right: index < tabs.length - 1 ? 8.0 : 0, // เว้นระยะระหว่างปุ่ม
              ),
  ...
  Widget _buildTabButton(int index, String title, FontSizeProvider fontProvider) {
  ...
  Widget _buildJobCard({
    ...
    return Consumer<FontSizeProvider>(
      builder: (context, fontProvider, child) {
        return GestureDetector(
      onTap: () async {
        print('Card tapped: $jobType'); // Debug print
